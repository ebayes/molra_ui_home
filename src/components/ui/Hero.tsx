"use client"

import { useState } from "react"
import { Button } from "../Button"
import AudioLabeling from "./AudioLabeling"
import HeroImage from "./HeroImage"
import ImageLabeling from "./ImageLabeling"

export default function Hero() {
  const [selectedButton, setSelectedButton] = useState("species")

  const handleButtonClick = (buttonName: string) => {
    setSelectedButton(buttonName)
  }

  const renderImage = () => {
    switch (selectedButton) {
      case "species":
        return <HeroImage />
      case "image":
        return <ImageLabeling />
      case "audio":
        return <AudioLabeling />
      default:
        return <HeroImage />
    }
  }

  return (
    <section
      aria-labelledby="hero-title"
      className="mt-32 flex flex-col items-center justify-center text-center sm:mt-40"
    >
      <h1
        id="hero-title"
        className="inline-block animate-slide-up-fade bg-gradient-to-br from-gray-900 to-gray-800 bg-clip-text p-2 text-4xl font-bold tracking-tighter text-transparent sm:text-6xl md:text-7xl dark:from-gray-50 dark:to-gray-300"
        style={{ animationDuration: "700ms" }}
      >
        The future of <br /> biodiversity monitoring
      </h1>
      <p
        className="mt-6 max-w-xl animate-slide-up-fade text-lg text-gray-700 dark:text-gray-400"
        style={{ animationDuration: "900ms" }}
      >
        The Map of Life XPrize team brings together scientists, engineers and local partners to build
        conservation tools for the AI era.
      </p>
      <div
        className="mt-8 flex w-full animate-slide-up-fade flex-col justify-center gap-3 px-3 sm:flex-row"
        style={{ animationDuration: "1100ms" }}
      >
        <Button
          variant={selectedButton === "species" ? "primary" : "secondary"}
          className="h-10 font-semibold"
          onClick={() => handleButtonClick("species")}
        >
          Species list
        </Button>

        <Button
          variant={selectedButton === "image" ? "primary" : "secondary"}
          className="h-10 font-semibold"
          onClick={() => handleButtonClick("image")}
        >
          Image labeling
        </Button>

        <Button
          variant={selectedButton === "audio" ? "primary" : "secondary"}
          className="h-10 font-semibold"
          onClick={() => handleButtonClick("audio")}
        >
          Audio labeling
        </Button>        
      </div>
      <div
      className="relative mx-auto ml-3 mt-20 h-fit w-[40rem] max-w-6xl animate-slide-up-fade sm:ml-auto sm:w-full sm:px-2"
      style={{ animationDuration: "1400ms" }}
    >
      {renderImage()}
      <div
        className="absolute inset-x-0 -bottom-20 -mx-10 h-2/4 bg-gradient-to-t from-white via-white to-transparent lg:h-1/4 dark:from-gray-950 dark:via-gray-950"
        aria-hidden="true"
      />
      </div>
    </section>
  )
}